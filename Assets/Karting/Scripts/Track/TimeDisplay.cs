...
namespace KartGame.Track
{
    ...
    public class TimeDisplay : MonoBehaviour
    {
        ...
        void DisplaySessionBestLapTime()
        {
            ...
            m_StringBuilder.Append('\n');
            //Codigo para guardar el mejor tiempo de la sesion y activar el mensaje new record si se mejora el tiempo
            if (PlayerPrefs.GetFloat("Time") == 0)
            {
                PlayerPrefs.SetFloat("Time", bestLapTime);
                textRecord.enabled = true;
            }
            if (bestLapTime < PlayerPrefs.GetFloat("Time"))
            {
                PlayerPrefs.SetFloat("Time", bestLapTime);
                textRecord.enabled = true;
